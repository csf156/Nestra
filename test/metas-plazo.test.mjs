...
test('nuevaFechaSugerida: un mes desde hoy', () => {
  assert.equal(nuevaFechaSugerida('2026-09-01'), '2026-10-01');
});

test('nuevaFechaSugerida: cruce de año', () => {
  assert.equal(nuevaFechaSugerida('2026-12-15'), '2027-01-15');
});

test('nuevaFechaSugerida: día 31 en un mes que no lo tiene → último día real', () => {
  assert.equal(nuevaFechaSugerida('2026-01-31'), '2026-02-28');
});

test('nuevaFechaSugerida: sin fecha de hoy válida → null', () => {
  assert.equal(nuevaFechaSugerida(''), null);
});
